...
export class AuxiliarMaterialesComponent implements OnInit {
  ...
  //materialesAsociadosAgenda: MaterialRequerido[];
  //dataMats = null;
  //parrafo = "";
  //estadosMat: estadoMatClass[];
  ...
  constructor(
    ...
    listarMateriales(){
      ...
      {
        this.materialesRequeridos = MaterialRequerido.fromJSON(rest);
        if(this.materialesRequeridos != null){
          this.convertirEstadoLleda(this.materialesRequeridos);
          this.listarMaterialesRequeridos();
        }else{ 
          this.materialesRequeridos=[];
          this.parrafo = "No hay materiales asociados al procedimiento";        
          this.notificationService.success('No hay materiales asosciados al procedimiento');
        }
    ...
    listarMaterialesRequeridos(){
      if(parseInt(this.idModalidad) != null){
        this.materialesService.getMaterialesProcedimiento(parseInt(this.idProcedimiento),parseInt(this.idModalidad)).subscribe(
          (result: MaterialRequerido[]) => this.materialesRequeridos = result);
      }else{
